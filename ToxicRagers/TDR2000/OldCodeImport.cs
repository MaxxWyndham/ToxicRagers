...
namespace reverseEngineerinator
{
    class gameTDR2000
    {
        static public void loopDirectoriesIn(string sPath)
        {
            foreach (DirectoryInfo d in new DirectoryInfo(sPath).GetDirectories())
            {
                loopDirectoriesIn(d.FullName);

                processPaksIn(d.FullName);
            }
        }

        static private void processPaksIn(string sPath)
        {
            foreach (FileInfo f in new DirectoryInfo(sPath).GetFiles("*.dir"))
            {
                processPak(f.FullName);
            }
        }

        static private void processPak(string sPath)
        {
            BinaryReader brDir;
            ...
            brDir = new BinaryReader(new FileStream(sPath, FileMode.Open));
            ...
            while (brDir.BaseStream.Position < brDir.BaseStream.Length)
            {
                ...
                extractFile(sPath.ToLower().Replace(".dir", ".pak"), sPath.Substring(0, sPath.LastIndexOf(".")), new string(filename, 0, i), (int)brDir.ReadUInt32(), (int)brDir.ReadUInt32());
        ...
        static private void extractFile(string source, string dest, string fileName, int offset, int size)
        {
            if (!Directory.Exists(dest)) { Directory.CreateDirectory(dest); }
            BinaryWriter bw = new BinaryWriter(new FileStream(dest + "\\" + fileName, FileMode.Create));
...
//        Public Sub New(ByVal s As String, ByVal i As Integer, ByVal j As Integer)
//            _name = s
//            _length = i
//            _offset = j
//        End Sub
...
//        For Each fi As IO.FileInfo In New IO.DirectoryInfo("D:\DDRaw\").GetFiles("*.pth")
//            parseAndExtract("D:\DDRaw\", fi.Name.Replace(fi.Extension, ""))
//        Next
...
//        Dim br As New IO.BinaryReader(New IO.FileStream(sFolder & sFileName & ".PTH", IO.FileMode.Open))
//        Do Until br.BaseStream.Position = br.BaseStream.Length
//            files.Add(New Resource(ReadString(br), br.ReadInt32(), br.ReadInt32()))
//            iSize += files(files.Count - 1).Length
//        Loop
//        br.Close()
...
//        Dim sPath As String = "D:\DDRaw\BOWL3S\BOWL3S.WWF"
//        'sPath = "D:\DDRaw\BOWL1\BOWL1.WWF"

//        Dim offsets As New List(Of Integer)
//        Dim iOffset, iOffsetA, iOffsetB As Integer
//        Dim iCurrentBlock, iSection, iSizeOfSection As Integer
//        Dim j As Integer

//        Dim br As New IO.BinaryReader(New IO.FileStream(sPath, IO.FileMode.Open))

//        Console.WriteLine(br.ReadUInt16())
//        Dim bNumOffsets As Byte = br.ReadByte() * br.ReadByte()

//        For i As Integer = 0 To bNumOffsets - 1
//            iOffset = br.ReadInt32()
//            If iOffset = 0 Then Continue For
//            offsets.Add(iOffset)
//            Console.WriteLine("MAPBLOCK " & i & " begins at " & offsets(i))
//        Next

//        ReadString(br, 8)

//        Do
//            iOffset = br.BaseStream.Position

//            j = 0
//            For Each i As Integer In offsets
//                If i = br.BaseStream.Position Then
//                    Console.WriteLine("Entering block " & j & " (offset " & i & ")")
//                    iSection = 0
//                    iCurrentBlock = j
//                    Application.DoEvents()
//                    Exit For
...
//            Console.WriteLine("Entering section " & iSection)

//            iOffsetA = br.ReadUInt16()
//            Console.WriteLine(iOffsetA & " (offset to list A)")
//            Console.WriteLine(br.ReadUInt16() & " (always 255?)")
//            iOffsetB = br.ReadUInt16()
//            Console.WriteLine(iOffsetB & " (offset to list B)")

//            Console.WriteLine(br.ReadUInt16() & " (???)")
//            Console.WriteLine(br.ReadUInt16() & " (???)")
//            Console.WriteLine(br.ReadUInt16() & " (???)")
//            Console.WriteLine(br.ReadUInt16() & " (???)")
//            iSizeOfSection = br.ReadUInt16()
//            Console.WriteLine(iSizeOfSection & " (size of section)")

//            'Dim lVert As New List(Of Vector3)

//            'For i As Integer = 0 To 59
//            '    ReadHalf(br)
//            '    'lVert.Add(New Vector3(br.ReadInt16(), br.ReadInt16(), br.ReadInt16()))
//            'Next

//            'br.BaseStream.Seek(iOffset + iOffsetA, IO.SeekOrigin.Begin)

//            'Dim lFace As New List(Of Face)
...
//            '    lFace.Add(New Face(a, b, c, 0, 0, 0, 0, 0))
//            '    lFace.Add(New Face(a, c, d, 0, 0, 0, 0, 0))
//            'Next

//            If iSizeOfSection > 0 Then
//                br.BaseStream.Seek(iOffset + iSizeOfSection, IO.SeekOrigin.Begin)
//            Else
//                If iCurrentBlock + 1 >= offsets.Count Then Exit Do
//                br.BaseStream.Seek(offsets(iCurrentBlock + 1), IO.SeekOrigin.Begin)
//            End If

//            Console.WriteLine("Seeking to " & br.BaseStream.Position)
//            iSection += 1

//            Application.DoEvents()
//        Loop While br.BaseStream.Position < br.BaseStream.Length

//        br.BaseStream.Seek(iOffset + iOffsetB, IO.SeekOrigin.Begin)
//        Console.WriteLine("Seeking to " & br.BaseStream.Position)
...
//        Console.WriteLine("Weird xx80 block has " & j & " entries")

//        br.BaseStream.Seek(-2, IO.SeekOrigin.Current) '<-- needed for some, not others, find out why

//        Console.WriteLine("Texture List: ")

//        Dim sTexName As String
//        Do
//            sTexName = ReadString(br, 8)
//            If sTexName.Length > 0 Then Console.WriteLine(br.BaseStream.Position - 8 & " : " & sTexName)
//        Loop While sTexName <> "n"

//        Console.WriteLine("Entering " & ReadString(br, 8))
...
//        Console.WriteLine(br.ReadByte() & " (???)")
//        Console.WriteLine(br.ReadInt32() & " (???)")
//        Console.WriteLine(br.ReadInt32() & " (length of header)")
...
//        br.BaseStream.Seek(11364, IO.SeekOrigin.Begin)
//        MsgBox(br.ReadSingle())
